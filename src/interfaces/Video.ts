...
export type VideoPartUploadInitResult = Result<VideoPartUploadInitData>

export interface VideoPartUploadUrlParam extends VideoUploadUrlParam {
    upload_id: string;// 分片上传的标记。有限时间为2小时。
    part_number?: number;// 表示该分片在整个视频内的相对位置，从1开始（1即表示第一段视频分片）。上传过程时，必传
}
...
export type VideoCreateResult = Result<VideoCreateResultData>