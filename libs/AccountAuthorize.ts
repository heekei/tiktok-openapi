import { AccesstokenParams, AccesstokenResult, AuthorizeV2Params, ClientTokenParams, ClientTokenResult, ConnectParams, RefreshTokenParams, RefreshTokenResult, RenewRefreshTokenParams, RenewRefreshTokenResult } from '../interfaces/AccountAuthorize';
import { Result } from '../interfaces/Common';
import { ajax } from '../utils/ajax';

export default class AccountAuthorize {
    /**
     * @description 获取授权码(code)
     * @summary 该接口只适用于抖音获取授权临时票据（code）。
     * @param {ConnectParams} params
     * @returns {Promise<Result>}
     * @memberof AccountAuthorize
     */
    connect(params: ConnectParams): Promise<Result> {
     ...
     * @memberof AccountAuthorize
     */
    accesstoken(data: AccesstokenParams): Promise<AccesstokenResult> {
     ...
     * @memberof AccountAuthorize
     */
    renewRefreshToken(data: RenewRefreshTokenParams): Promise<RenewRefreshTokenResult> {
     ...
     * @memberof AccountAuthorize
     */
    clientToken(data: ClientTokenParams): Promise<ClientTokenResult> {
     ...
     * @memberof AccountAuthorize
     */
    refreshToken(data: RefreshTokenParams): Promise<RefreshTokenResult> {
     ...
     * @memberof AccountAuthorize
     */
    authorizeV2(params: AuthorizeV2Params): Promise<Result> {
